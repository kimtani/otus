...
#### Шаг 7. Проверьте наличие сквозного подключения. (не проверено)
...
- Данный MAC-адрес уже изучен и хранится в NVRAM. 
...
-  DHCP snooping настроен для VLAN10,  а пул адресов для раздачи на роутере исключает адреса этого VLAN, так как это VLAN управления. DHCP пул настроен для обычных клиентов - юзеров.

3. Что касается безопасности порта, в чем разница между типом абсолютного устаревания и типом
устаревание по неактивности?

- При абсолютном устаревании адреса не удаляются, даже если они неактивны при заданном времени. А по неактивности удаляются, если в указанном времени адреса на порту не использовались.
